...
mod tests {
    ...
    #[tokio::test]
    async fn customer_crud_flow_works() {
        ...
        let listed = list_customers_impl(&pool, Some("اختبار".to_owned()))
            .await
            .expect("customers should list");
        assert_eq!(listed.len(), 1);

        let fetched = get_customer_by_id(&pool, created.id)
            .await
        ...
        let deleted = delete_customer_impl(&pool, created.id)
            .await
            .expect("delete should succeed");
        assert!(deleted);

        let listed_after_delete = list_customers_impl(&pool, Some("محدث".to_owned()))
            .await
            .expect("customers should list after delete");
